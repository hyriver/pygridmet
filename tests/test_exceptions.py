...
def test_invalid_variable():
    with pytest.raises(InputValueError) as ex:
        _ = gridmet.get_bycoords(COORDS, DATES, variables="tt")
    assert "variables" in str(ex.value)


def test_invalid_coords():
    with pytest.raises(InputRangeError) as ex:
        _ = gridmet.get_bycoords((0, 0), DATES)
    assert "-124.7666" in str(ex.value)


def test_invalid_date():
    with pytest.raises(InputRangeError) as ex:
        _ = gridmet.get_bycoords(COORDS, ("1950-01-01", "2010-01-01"))
    assert "1980" in str(ex.value)


def test_invalid_year():
    with pytest.raises(InputRangeError) as ex:
        _ = gridmet.get_bycoords(COORDS, 1950)
    assert "1980" in str(ex.value)
...
def test_invalid_date_tuple():
    with pytest.raises(InputTypeError) as ex:
        _ = gridmet.get_bycoords(COORDS, ("2010-01-01"))
